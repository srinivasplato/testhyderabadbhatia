    ...
    <div class="col-md-12">
      ...
      <div class="panel panel-default">
        ...
        <div class="col-md-6 col-xs-6">
          <h2><span class="fa fa-list"></span> Faculty</h2>
        </div>
         <div class="col-md-6 col-xs-6">
          <h3 class="panel-title" style="float: right;"><a href="<?=base_url();?>admin/register/add_faculty" class="btn btn-success">Add</a></h3>
        </div> 
        </div>
        <div class="panel-body">
          ...
          <div id="wait" style="display:none;width:69px;height:89px;position:absolute;top:50%;left:50%;padding:2px; z-index:99999999"><img src='<?=base_url();?>assets/img/demo_wait.gif' width="64" height="64" /></div>
            ...
            <table id="faculty" class="table datatable table-striped">
            ...
            <form id="fees_form" name="search_fees" method="post" class="pull-right">
              ...
              <div class="col-md-2">
                  <select name="search_on_1" id="search_on_1" class="form-control input-sm custom-input">
                     <option value="1">ExamName</option>
                      <option value="2">Name</option>
                      <option value="3">TITLE</option>
                      <option value="4">SPECIALISATION</option>
                  </select>
              ...
              <div class="col-md-2">
              <button type="button" id="search_faculty" class="btn btn-info margin_search" style=""><i class="fa fa-search icon-style"></i></button>
              <a class="btn btn-danger" style="display:none;" id="searchreset" href="<?php echo base_url('admin/register/faculty'); ?>"><li class="fa fa-minus icon-style"></li></a>
              </div>
            </form> 
            <!-------- /Search Form ---------->
...
<script>
    ...
    $(document).ready(function () {
        ...
        "destroy": true,
        "ajax": {
            "url": "<?php echo base_url('admin/register/all_faculty'); ?>",
            "type":"POST",
            beforeSend: function() {
        ...
        "columns": [
            ...
            { "title": "exam name", "name":"ename","orderable": false, "data":"ename", "width":"0%" },
            { "title": "Name", "name":"name","orderable": false, "data":"name", "width":"0%" },
            { "title": "Title", "title":"title","orderable": false, "data":"title", "width":"0%" },
            { "title": "specialisation", "name":"specialisation","orderable": false, "data":"specialisation", "width":"0%" },

            { "title": "Image", "name":"profile_img","orderable": false, "data":"profile_img", "width":"0%" },
            { "title": "Created Date", "name":"created_on","orderable": false, "data":"created_on", "width":"0%" },
        ],
        "fnCreatedRow": function(nRow, aData, iDataIndex) 
        {

          var image = '<img src="'+aData['profile_img']+'" height="50" width="50">';
         
          $(nRow).find('td:eq(7)').html(image);

          var status ='<a href="'+url+'admin/register/delete_faculty/'+aData['id']+'" class="btn btn-danger btn-condensed" title="Delete faculty"><i class="fa fa-trash"></i></a>';
          $(nRow).find('td:eq(1)').html(status);

          if(aData['status'] == "Active")
          {
            var status ='<a title="Click to Inactive" href="'+url+'admin/register/change_faculty_status/'+aData['id']+'/Inactive" class="btn btn-success btn-condensed">Active</a>';
          }
          else
          {
            var status ='<a title="Click to Active" href="'+url+'admin/register/change_faculty_status/'+aData['id']+'/Active" class="btn btn-danger btn-condensed">In Active</a>';
          }
         //$(nRow).find('td:eq(2)').html(status);

          var action ='<a onclick="return confirm('+"'Are you sure want to delete?'"+');" class="btn btn-danger btn-condensed" title="delete" href="'+url+'admin/register/delete_faculty/'+aData['id']+'"><i class="fa fa-trash"></i></a>';
