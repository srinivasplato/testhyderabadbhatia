    ...
    <div class="col-md-12">
      ...
      <div class="panel panel-default">
        ...
        <div class="col-md-6 col-xs-6">
          <h2><span class="fa fa-edit"></span>Packages</h2>
        </div>
        <div class="col-md-6 col-xs-6">
          <h3 class="panel-title" style="float: right;"><a href="<?=base_url();?>admin/register/add_packages" class="btn btn-success">Add</a></h3>
        </div>
        </div>
        <div class="panel-body">
          ...
          <div id="wait" style="display:none;width:69px;height:89px;position:absolute;top:50%;left:50%;padding:2px; z-index:99999999"><img src='<?=base_url();?>assets/img/demo_wait.gif' width="64" height="64" /></div>
            ...
            <form id="fees_form" name="search_fees" method="post" class="pull-right">
              <div class="col-md-3 col-md-offset-3" style="padding:0">
                ...
                <div class="col-md-2">
                  <select name="search_on_1" id="search_on_1" class="form-control input-sm custom-input">
                      <option value="1">Package name</option>
                  </select>
              ...
              <a class="btn btn-danger" style="display:none;" id="searchreset" href="<?php echo base_url('admin/register/packages'); ?>"><li class="fa fa-minus icon-style"></li></a>
              </div>
            </form> 
            <!-------- /Search Form ---------->
...
<script>
    ...
    $(document).ready(function () {
        ...
        "destroy": true,
        "ajax": {
            "url": "<?php echo base_url('admin/register/all_packages'); ?>",
            "type":"POST",
            beforeSend: function() {
        ...
        "columns": [
            ...
            { "title": "Package Name", "name":"name","orderable": false, "data":"package_name", "width":"0%" },
            { "title": "Order", "name":"order","orderable": false, "data":"order", "width":"0%" },

           // { "title": "Course Names", "name":"name","orderable": false, "data":"course_names", "width":"0%" },
            { "title": "Created Date", "name":"created_on","orderable": false, "data":"created_on", "width":"0%" },  

            { "title": "Update to Users", "name":"name","orderable": false, "data":"id", "width":"0%" },
        ...
        {
          var package ='<a class="btn btn-warning btn-condensed" title="edit" href="'+url+'admin/register/update_package_to_users/'+aData['id']+'"><i class="fa fa-edit"></i> Update to users</a>';
          $(nRow).find('td:eq(4)').html(package);

        var view ='<a title="Click to View" href="'+url+'admin/register/view_packages/'+aData['id']+'" class="btn btn-success btn-condensed">View</a>';
        $(nRow).find('td:eq(5)').html(view);   

          var action ='<a class="btn btn-warning btn-condensed" title="edit" href="'+url+'admin/register/edit_packages/'+aData['id']+'"><i class="fa fa-edit"></i></a>';
          $(nRow).find('td:eq(7)').html(action);

          if(aData['status'] == "Active")
          {
            var status ='<a title="Click to Inactive" href="'+url+'admin/register/change_packages_status/'+aData['id']+'/Inactive" class="btn btn-success btn-condensed">ON</a>';
          }
          else
          {
            var status ='<a title="Click to Active" href="'+url+'admin/register/change_packages_status/'+aData['id']+'/Active" class="btn btn-danger btn-condensed">OFF</a>';
          }
