    ...
    <div class="col-md-12">
      ...
      <div class="panel panel-default">
        ...
        <div class="col-md-6 col-xs-6">
         ...
         <div class="col-md-6 col-xs-6">
          <h3 class="panel-title" style="float: right;"><a href="<?=base_url();?>admin/register/add_banners" class="btn btn-success">Add</a></h3>
        </div> 
        </div>
        <div class="panel-body">
          ...
          <div id="wait" style="display:none;width:69px;height:89px;position:absolute;top:50%;left:50%;padding:2px; z-index:99999999"><img src='<?=base_url();?>assets/img/demo_wait.gif' width="64" height="64" /></div>
            ...
            <table id="banners" class="table datatable table-striped">
            ...
            <form id="fees_form" name="search_fees" method="post" class="pull-right">
              ...
              <div class="col-md-2">
              <button type="button" id="search_banners" class="btn btn-info margin_search" style=""><i class="fa fa-search icon-style"></i></button>
              <a class="btn btn-danger" style="display:none;" id="searchreset" href="<?php echo base_url('admin/register/banners'); ?>"><li class="fa fa-minus icon-style"></li></a>
              </div>
            </form> 
            <!-------- /Search Form ---------->
...
<script>
    ...
    $(document).ready(function () {
        ...
        "destroy": true,
        "ajax": {
            "url": "<?php echo base_url('admin/register/all_banners'); ?>",
            "type":"POST",
            beforeSend: function() {
        ...
        "columns": [
            ...
            { "title": "Actions", "name":"action", "orderable": false, "deafultContent":"", "data": "id", "width":"0%", "class":"td_action"},
              { "title": "Exam name", "name":"name","orderable": false, "data":"name", "width":"0%" },
              { "title": "order", "name":"order","orderable": false, "data":"order", "width":"0%" },
              { "title": "Youtube Url", "name":"name","orderable": false, "data":"youtube_video_url", "width":"0%" },
        ...
        "fnCreatedRow": function(nRow, aData, iDataIndex) 
        {
          ...
          $(nRow).find('td:eq(4)').html(image);

          var status ='<a href="'+url+'admin/register/delete_banners/'+aData['id']+'" class="btn btn-danger btn-condensed" title="Delete banners"><i class="fa fa-trash"></i></a><a href="'+url+'admin/register/edit_banners/'+aData['id']+'" class="btn btn-primary btn-condensed" title="Delete banners"><i class="fa fa-edit"></i></a>';
         // $(nRow).find('td:eq(1)').html(status);

          if(aData['status'] == "Active")
          {
            var status ='<a title="Click to Inactive" href="'+url+'admin/register/change_banners_status/'+aData['id']+'/Inactive" class="btn btn-success btn-condensed">Active</a>';
          }
          else
          {
            var status ='<a title="Click to Active" href="'+url+'admin/register/change_banners_status/'+aData['id']+'/Active" class="btn btn-danger btn-condensed">In Active</a>';
          }
         //$(nRow).find('td:eq(2)').html(status);

          var action ='<a onclick="return confirm('+"'Are you sure want to delete?'"+');" class="btn btn-danger btn-condensed" title="delete" href="'+url+'admin/register/delete_banners/'+aData['id']+'"><i class="fa fa-trash"></i></a><a href="'+url+'admin/register/edit_banners/'+aData['id']+'" class="btn btn-warning btn-condensed" title="Edit banners"><i class="fa fa-edit"></i></a>';
