    ...
    <div class="col-md-12">
      ...
      <div class="panel panel-default">
        ...
        <div class="col-md-6 col-xs-6">
          <h2><span class="fa fa-edit"></span>Courses/Bhatia Batchs</h2>
        </div>
        <div class="col-md-6 col-xs-6">
          <!-- <h3 class="panel-title" style="float: right;"><a href="<?=base_url();?>admin/register/add_exams" class="btn btn-success">Add</a></h3> -->
        </div>
        </div>
        <div class="panel-body">
          ...
          <div id="wait" style="display:none;width:69px;height:89px;position:absolute;top:50%;left:50%;padding:2px; z-index:99999999"><img src='<?=base_url();?>assets/img/demo_wait.gif' width="64" height="64" /></div>
            ...
            <form id="fees_form" name="search_fees" method="post" class="pull-right">
              ...
              <a class="btn btn-danger" style="display:none;" id="searchreset" href="<?php echo base_url('admin/register/exams'); ?>"><li class="fa fa-minus icon-style"></li></a>
              </div>
            </form> 
            <!-------- /Search Form ---------->
...
<script>
    ...
    $(document).ready(function () {
        ...
        "destroy": true,
        "ajax": {
            "url": "<?php echo base_url('admin/register/all_exams'); ?>",
            "type":"POST",
            beforeSend: function() {
        ...
        "columns": [
          ...
          //  { "title": "Actions", "name":"action", "orderable": false, "deafultContent":"", "data": "id", "width":"0%", "class":"td_actionn"},
            { "title": "Course/Batch Name", "name":"name","orderable": false, "data":"name", "width":"0%" },

            { "title": "Student Code", "name":"image","orderable": false, "data":"student_code", "width":"0%" },
            ...
            { "title": "Assign Users", "name":"id","orderable": false, "data":"id", "width":"0%" },
        ...
        "fnCreatedRow": function(nRow, aData, iDataIndex) 
        {
          ...
          var add ='<a title="Click to Inactive" href="'+url+'admin/register/assignCourseToUser/'+aData['id']+'" class="btn btn-success btn-condensed">Assign Users</a>';
           

           $(nRow).find('td:eq(6)').html(add);
          var image_path= url+aData['image'];
          //alert(image_path);
          var file_status=doesFileExist(image_path);
//alert(file_status);
         if((aData['image'] != '') && (file_status == true) ){
         var image = '<img src="'+aData['image']+'" height="50" width="50">';
            }else{
         ...
         // $(nRow).find('td:eq(2)').html(image);
          ...
          var action ='<a class="btn btn-warning btn-condensed" title="edit" href="'+url+'admin/register/edit_exams/'+aData['id']+'"><i class="fa fa-edit"></i></a> <a onclick="return confirm('+"'Are you sure want to delete?'"+');" class="btn btn-danger btn-condensed" title="delete" href="'+url+'admin/register/delete_exams/'+aData['id']+'"><i class="fa fa-trash"></i></a>';
          //$(nRow).find('td:eq(1)').html(action);

          if(aData['status'] == "Active")
          {
            var status ='<a title="Click to Inactive" href="'+url+'admin/register/change_exam_status/'+aData['id']+'/Inactive" class="btn btn-success btn-condensed">ON</a>';
          }
          else
          {
            var status ='<a title="Click to Active" href="'+url+'admin/register/change_exam_status/'+aData['id']+'/Active" class="btn btn-danger btn-condensed">OFF</a>';
          }
